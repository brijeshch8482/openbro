...
def test_capability_for_known_models():
    ...
    assert _capability_for("qwen2.5-coder:7b") <= 65  # coder = poor at agent
...
def test_probe_returns_at_least_cloud_options():
    """Even with no Ollama installed, cloud options should appear."""
...
def test_probe_includes_ollama_models():
    with patch(
        "openbro.llm.auto_select._ollama_installed_models",
        return_value=["llama3.1:8b", "qwen2.5-coder:7b"],
    ):
        candidates = probe_available()
    ollama_models = [c["model"] for c in candidates if c["provider"] == "ollama"]
...
def test_best_available_with_no_keys():
    """Without any API keys, no cloud is available; if no Ollama either, returns None."""
    with patch("openbro.llm.auto_select._ollama_installed_models", return_value=[]):
...
def test_suggest_upgrade_offers_better_model():
    with patch("openbro.llm.auto_select._ollama_installed_models", return_value=[]):
        ...
        result = suggest_upgrade(("ollama", "phi3:mini"), cfg)
    assert result is not None
